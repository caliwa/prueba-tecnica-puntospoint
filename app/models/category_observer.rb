class CategoryObserver < ActiveRecord::Observer
  def after_create(category)
    log_event(category, "created", nil, category.attributes)
  end

  def after_update(category)
    old_values = category.previous_changes.transform_values(&:first)
    new_values = category.previous_changes.transform_values(&:second)
    log_event(category, "updated", old_values, new_values)
  end

  def before_destroy(category)
    log_event(category, "deleted", category.attributes, nil)
  end
